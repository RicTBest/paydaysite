...
export class PlayoffDataService {
  ...
  // Check if we're actually in playoff time for the given season
  static async isPlayoffTime(season = 2025) {
    try {
      // Get current NFL week from ESPN
      const response = await axios.get(`${ESPN_BASE_URL}/scoreboard`)
      const data = response.data
      
      const currentSeason = data.season?.year
      const currentWeek = data.week?.number
      const seasonType = data.season?.type // 2 = regular, 3 = playoffs
      
      console.log(`Current NFL status: Season ${currentSeason}, Week ${currentWeek}, Type ${seasonType}`)
      
      // We're in playoff time if:
      // 1. seasonType is 3 (playoffs) AND it's the right season, OR
      // 2. We're past week 17 of the regular season for this season
      
      if (seasonType === 3 && currentSeason === season) {
        return { ready: true, reason: 'Currently in playoffs' }
      }
      
      if (seasonType === 2 && currentSeason === season && currentWeek >= 18) {
        return { ready: true, reason: `Week ${currentWeek} - regular season complete` }
      }
      
      // Not playoff time yet
      return { 
        ready: false, 
        reason: `Too early - currently Week ${currentWeek} of ${currentSeason} regular season. Playoffs start after Week 18.`,
        currentSeason,
        currentWeek,
        seasonType
      }
    } catch (error) {
      console.error('Error checking playoff time:', error)
      // Default to not ready if we can't determine
      return { ready: false, reason: 'Could not determine current NFL week' }
    }
  }

  // Main function to run all playoff updates
  static async updateAllPlayoffs(season = 2025, force = false) {
    const summary = {
      berths: null,
      wildCard: null,
      divisional: null,
    ...
    try {
      // Safety check: Don't run if we're not in playoff time (unless forced)
      if (!force) {
        const playoffCheck = await this.isPlayoffTime(season)
        if (!playoffCheck.ready) {
          console.log(`Skipping playoff updates: ${playoffCheck.reason}`)
          summary.skippedReason = playoffCheck.reason
          return summary
        }
        console.log(`Playoff time confirmed: ${playoffCheck.reason}`)
      } else {
        console.log('Force flag set - skipping playoff time check')
      }
