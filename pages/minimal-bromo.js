...
export default function MinimalBromoScoreboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log('Auto-refreshing minimal bromo scoreboard...')
  ...
  async function loadData() {
    console.log('=== STARTING MINIMAL BROMO SCOREBOARD DATA LOAD ===')
    ...
    try {
      ...
      if (selectedWeek === 'overall') {
        ...
        await loadOverallData(teams, owners, teamLookup, ownerLookup, teamToOwnerLookup)
      } else {
        // Load specific week data
        await loadWeekData(teams, owners, teamLookup, ownerLookup, teamToOwnerLookup, selectedWeek)
      }
      
    } catch (error) {
      console.error('Error in minimal bromo loadData:', error)
      setLoading(false)
    }
  }

  async function loadOverallData(teams, owners, teamLookup, ownerLookup, teamToOwnerLookup) {
    console.log('Loading bromo overall data across all weeks...')
    ...
    // Initialize team stats for bromo teams only
    const teamStats = {}
    ...
    // Process all awards - only for bromo teams
    allAwards.forEach(award => {
      const team = teamStats[award.team_abbr]
      if (!team) return // Skip non-bromo teams
    ...
    // Process game wins (only count wins not already counted by awards) - only for bromo teams
    const winsByWeek = {}
    allAwards.forEach(award => {
      if (award.type === 'WIN' || award.type === 'TIE_AWAY') {
  ...
  async function loadWeekData(teams, owners, teamLookup, ownerLookup, teamToOwnerLookup, week) {
    console.log('Loading bromo week data for week:', week)
    ...
    // 5. Initialize team stats for bromo teams only
    const teamStats = {}
    ...
    // 6. Process awards - only for bromo teams
    ...
    // 7. Add wins from completed games - only for bromo teams
  ...
  function buildOwnerRankings(teamStats) {
    // 8. Calculate bromo owner totals and team earnings
    const ownerTotals = {}
    ...
    console.log('=== MINIMAL BROMO SCOREBOARD LOAD COMPLETE ===')
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex justify-center items-center">
        <div className="text-center">
          ...
          <div className="text-sm font-semibold text-gray-800">Loading Bromo...</div>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-3">
          ...
          >
            ← Back to Bromo League
          </a>
          <h1 className="text-xl font-bold text-gray-900">
            {selectedWeek === 'overall' ? 'Bromo Overall Standings' : `Bromo Week ${selectedWeek}`}
          </h1>
          <select
            value={selectedWeek}
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {owner.teams.map((team) => (
                  <div key={team.abbr} className="flex items-center space-x-1 bg-blue-50 border border-blue-200 rounded px-2 py-1">
                    <img 
      ...
      {ownerRankings.length === 0 && (
        <div className="text-center py-12">
          <div className="text-lg font-bold text-gray-700 mb-2">No Bromo Data</div>
          <p className="text-gray-600">No bromo data available for {selectedWeek === 'overall' ? 'overall standings' : `Week ${selectedWeek}`}</p>
        </div>
      )}
    </div>
  )
