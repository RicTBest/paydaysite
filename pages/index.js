...
export default function Home() {
  ...
  // Format names with proper corporate titles
  function formatCorporateName(name) {
    const nameLower = name.toLowerCase()
    if (nameLower.includes('will')) return 'W. Feldman, Esq.'
    ...
    if (nameLower.includes('joey')) return 'J.H. Rosenberg, Esq.'
    if (nameLower.includes('zack')) return 'Z. Reneau-Wedeen, MBA'
    return name
  }
        ...
        <div className="space-y-4">
          {leaderboard.map((owner, index) => {
            ...
            return (
              ...
              >
                <div className="p-3 sm:p-4 bg-gray-50 border-b-2 border-gray-300">
                    ...
                    <div className="flex items-start gap-3 sm:gap-4 w-full">
                      <div className="flex-shrink-0">
                        <img 
                          ...
                          className="w-16 h-16 sm:w-20 sm:h-20 object-cover border-2 border-gray-400"
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-gray-500 uppercase mb-1">Feldman Party Attendee</div>
                        <h2 className="text-base sm:text-lg font-bold text-gray-800 break-words">{formatCorporateName(owner.name)}</h2>
                        <div className="text-xs sm:text-sm text-gray-600 mt-1">
                          <div>Ranking: <span className="font-bold">#{rank}</span></div>
                          <div>Points: <span className="font-bold">{owner.totalEarnings}</span></div>
                          {numGooses > 0 && <div>Incidents: {numGooses}</div>}
                        </div>
                        <div className="text-xs text-gray-500 mt-2">
                          {isWill && "Currently engaging in discussions regarding Sonoma real estate trends."}
                          {!isWill && "Participating in work-approved networking initiatives."}
                        </div>
                      </div>
                    </div>
                    
                    <div className="w-full overflow-x-auto">
                      <table className="text-xs text-gray-600 border-collapse border border-gray-300 w-full">
                        <tbody>
                          <tr className="border-b border-gray-300 bg-white">
                            <td colSpan="2" className="py-1 px-2 font-semibold text-center">Key Performance Indicators</td>
                          </tr>
                          <tr className="border-b border-gray-300 bg-white">
                            <td className="py-1 px-2 border-r border-gray-300">Wins</td>
                            <td className="py-1 px-2 font-semibold text-right">{owner.wins}</td>
                          </tr>
                          <tr className="border-b border-gray-300 bg-white">
                            <td className="py-1 px-2 border-r border-gray-300">OBO</td>
                            <td className="py-1 px-2 font-semibold text-right">{owner.obo}</td>
                          </tr>
                          <tr className="border-b border-gray-300 bg-white">
                            <td className="py-1 px-2 border-r border-gray-300">DBO</td>
                            <td className="py-1 px-2 font-semibold text-right">{owner.dbo}</td>
                          </tr>
                          <tr className="bg-white">
                            <td className="py-1 px-2 border-r border-gray-300">EOY</td>
                            <td className="py-1 px-2 font-semibold text-right">{owner.eoy}</td>
                          </tr>
                        </tbody>
                      </table>
                      
                      <div className={`text-xs mt-2 px-2 py-1 border w-full ${
