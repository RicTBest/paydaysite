...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [
      ...
      fetch(`${baseUrl}/api/kalshi-probabilities?week=17&season=${season}`),
      fetch(`${baseUrl}/api/kalshi-probabilities?week=18&season=${season}`),
      fetch(`${baseUrl}/api/games?week=17&season=${season}`),
      fetch(`${baseUrl}/api/games?week=18&season=${season}`)
    ])
    ...
    // Returns: 1 if team won, 0 if team lost, 0.5 if tie, null if game not finished / not found
    ...
    const P_giants_w18 = getWinProb(week18Probs, week18Games, 'NYG') // Giants win vs Cowboys
    const P_raiders_w18 = getWinProb(week18Probs, week18Games, 'LV') // Raiders win vs Chiefs
    ...
    Object.entries(teamOwners).forEach(([team, owner]) => {
      ...
      const teamProb =
        team === 'NYG' ? P_giants : team === 'LV' ? P_raiders : team === 'NYJ' ? P_jets : P_browns
    ...
    const result = {
      ...
      season,
      gameStatuses, // null = not finished, 0 = lost, 1 = won, 0.5 = tie
      inputProbabilities: {
        week17: {
      ...
      tiebreakerOrder: ['CLE', 'NYG', 'NYJ', 'LV'],
      notes: [
